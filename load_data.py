...
@cache_results(_cache_fp='cache/duiener',_refresh=False)
def load_duie_ner(path,char_embedding_path=None,bigram_embedding_path=None,index_token=True,train_clip=False,
                       char_min_freq=1,bigram_min_freq=1,only_train_min_freq=0):
    from fastNLP.io.loader import ConllLoader
    from utils import get_bigrams
    if train_clip:
        train_path = os.path.join(path, 'train_dev.char.bio_clip1')
        test_path = os.path.join(path, 'test.char.bio_clip1')
    else:
        train_path = os.path.join(path,'train_dev.char.bio')
...
@cache_results(_cache_fp='cache/clue2020', _refresh=False)
def load_clue_ner(path, char_embedding_path=None, bigram_embedding_path=None, index_token=True, train_clip=True,
                  char_min_freq=1, bigram_min_freq=1, only_train_min_freq=0):
    from fastNLP.io.loader import ConllLoader
    from utils import get_bigrams
    if train_clip:
